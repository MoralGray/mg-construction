...
async function main() {
    ...
    if (roadCount > 0) {
        console.log(`Database already has ${roadCount} roads, skipping seed`);
        await prisma.$disconnect();
        return;
    }

    console.log('Database empty — seeding roads and work types');
    await prisma.road.createMany({ data: ROADS });
    console.log(`Seeded ${ROADS.length} roads`);

    await prisma.workType.createMany({ data: WORK_TYPES });
...
main().catch((e) => {
    console.error('Docker seed failed:', e);
    process.exit(1);
});
